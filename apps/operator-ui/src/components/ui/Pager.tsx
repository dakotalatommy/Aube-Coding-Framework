export default function Pager({ page, pageSize, total, onPrev, onNext }:{ page:number; pageSize:number; total:number; onPrev:()=>void; onNext:()=>void }){
  ...
  return (
    <nav className="flex items-center justify-between text-xs sticky bottom-[calc(var(--bvx-commandbar-height,64px)+env(safe-area-inset-bottom,0px)+8px)] bg-white/70 backdrop-blur rounded-md px-2 py-1 border" role="navigation" aria-label="Pagination">
