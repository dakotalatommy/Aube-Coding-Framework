...
type Props = {
  ...
  maxWidthClass?: string; // e.g., max-w-[560px] or max-w-[720px]
  minHeightClass?: string; // e.g., min-h-[560px]
  padded?: boolean;
};

export default function CenteredCard({ children, maxWidthClass = 'max-w-[560px]', minHeightClass = 'min-h-[560px]', padded = true }: Props) {
  return (
    <div className="min-h-[100svh] grid place-items-center px-4">
      <div className={`w-full ${maxWidthClass} ${minHeightClass} group rounded-2xl border-[3px] border-white/60 shadow-[0_24px_48px_-22px_rgba(0,0,0,0.25)] bg-white/70 backdrop-blur relative overflow-hidden` + (padded ? ' p-7 md:p-8' : '')}>
        ...
        <div className="relative z-10 max-h-[calc(100svh-160px)] overflow-auto">
          {children}
        </div>
      </div>
    </div>
