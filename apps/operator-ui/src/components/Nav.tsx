    ...
    <div className="sticky top-0 z-10">
      <nav className="max-w-5xl mx-auto mt-3 px-4 py-3 flex items-center justify-between gap-3 rounded-2xl border bg-white/70 backdrop-blur supports-[backdrop-filter]:bg-white/50 shadow-md" role="navigation" aria-label="Primary">
        <div className="flex items-center gap-2">
          <Link to="/" className="hidden md:inline-flex items-center gap-2 text-slate-900 font-semibold" style={{fontFamily:'\"Space Grotesk\", Inter, ui-sans-serif'}}>BrandVX</Link>
        </div>
        <div className="flex items-center gap-2 flex-wrap overflow-x-auto">
          {tabs.map(t=>{
            const active = loc.pathname === t.to;
            return (
              <Tooltip.Root key={t.to}>
                <Tooltip.Trigger asChild>
                  <Link
                    to={t.to}
                    aria-current={active ? 'page' : undefined}
                    className={`px-3 py-2 rounded-lg text-sm text-slate-900 transition ${active ? 'bg-gradient-to-r from-pink-50 to-sky-50 border border-pink-100/70 shadow-sm' : 'hover:bg-slate-50 focus-visible:ring-2 focus-visible:ring-pink-200'}`}
                  >
                    <span>{t.label}</span>
        ...
        <div className="flex items-center gap-2">
          ...
          <Link to="/login" className="px-3 py-2 rounded-lg border border-slate-200 bg-white text-slate-900 hover:bg-slate-50 shadow-sm">Sign in</Link>
          <Link to="/onboarding" className="px-3 py-2 rounded-lg text-white shadow hover:shadow-md bg-gradient-to-r from-pink-500 to-violet-500 hover:from-pink-600 hover:to-violet-600">Get started</Link>
