...
export default function Integrations(){
  const [settings, setSettings] = useState<any>({ tone:'helpful', services:['sms','email'], auto_approve_all:false, quiet_hours:{ start:'21:00', end:'08:00' } });
  ...
  const save = async () => {
    try{ setBusy(true); const r = await api.post('/settings',{ tenant_id: await getTenant(), ...settings }); setStatus(JSON.stringify(r)); }
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        ...
        <button className="text-sm text-slate-600 hover:underline" aria-label="Open integrations guide" onClick={()=>{
