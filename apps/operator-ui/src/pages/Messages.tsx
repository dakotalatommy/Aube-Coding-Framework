        ...
        <>
          ...
          <div className="rounded-2xl p-4 bg-white/60 backdrop-blur border border-white/70 shadow-sm">
            ...
            {!!quiet?.start && !!quiet?.end && (
              <div className="mb-2 text-xs text-slate-700 bg-slate-50 border border-slate-200 rounded-md px-2 py-1 inline-block">Quiet hours: {quiet.start}–{quiet.end}. Sending will be disabled during this window.</div>
            )}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3" role="form" aria-label="Send message form">
              <Input placeholder="contact_id" value={send.contact_id} onChange={e=>setSend({...send,contact_id:e.target.value})} />
            ...
            <div className="mt-2 text-xs text-slate-700 flex flex-wrap items-center gap-2">
              ...
              <button className="px-2 py-1 rounded-md border bg-white hover:bg-slate-50" onClick={()=> setQuiet({ start:'21:00', end:'08:00' })}>Suggest 21:00–08:00</button>
              <button className="px-2 py-1 rounded-md border bg-white hover:bg-slate-50" onClick={async()=>{ try { await api.post('/settings', { tenant_id: await getTenant(), quiet_hours: quiet }); } catch{} }}>Save</button>
            </div>
            <div className="mt-2 flex flex-wrap gap-2 text-xs">
              {presets.map(p=> (
                <button key={p.label} className="border rounded-full px-3 py-1 bg-white/80 backdrop-blur hover:shadow-sm" onClick={()=>setSend(s=>({...s, body: p.body }))}>{p.label}</button>
              ))}
            </div>
            ...
            <Button variant="primary" className="mt-2" onClick={sendMsg} disabled={isWithinQuiet(quiet)}>Send</Button>
          </div>

          <pre className="whitespace-pre-wrap text-sm text-slate-700">{status}</pre>
          {items.length === 0 ? (
            <div className="rounded-2xl bg-white/60 backdrop-blur border border-white/70 shadow-sm p-6 text-center text-slate-600">
              No messages yet. Try "Simulate SMS" to generate a sample message respecting STOP/HELP and consent.
            </div>
          ) : (
            <Table>
              <THead>
