...
export async function startOAuth(provider: 'square'|'acuity'|'hubspot'|'google'|'facebook'|'instagram'|'shopify', opts?: { returnTo?: 'onboarding'|'workspace' }){
  ...
  try {
    const first = await api.get(`/oauth/${provider}/login${opts?.returnTo?`?return=${encodeURIComponent(opts.returnTo)}`:''}`, { includeTenant: false });
    const url = String(first?.url||'');
    if (url) { window.location.href = url; return; }
  } catch {}
  try {
    const params = opts?.returnTo ? `?return=${encodeURIComponent(opts.returnTo)}` : '';
    const j = await api.get(`/oauth/${provider}/start${params}`, { includeTenant: false });
    const url = (j && j.url) || '';
    if (url) { window.location.href = url; return; }
  } catch {}
}
