...
export function ClientRemindersProvider({ children }: { children: ReactNode }) {
  ...
  const addReminder = (newReminder: Omit<ClientReminder, 'id'>) => {
    setReminders(prevReminders => {
      const nextId = prevReminders.length
        ? Math.max(...prevReminders.map(reminder => reminder.id)) + 1
        : 1
      return [...prevReminders, { ...newReminder, id: nextId }]
    })
  }
  ...
  const value = useMemo<ClientRemindersContextType>(
    () => ({
      reminders,
      addReminder,
      updateReminder,
  ...
  return <ClientRemindersContext.Provider value={value}>{children}</ClientRemindersContext.Provider>
}
