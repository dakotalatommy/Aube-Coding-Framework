...
const ModelViewerElement = (props: Record<string, any>) => createElement('model-viewer' as any, props)
...
export default function LandingV2() {
  ...
  useEffect(() => {
    ...
    if (!el) {
      console.info('[landing] model-viewer not mounted yet')
      return
    }
    const handleLoaded = () => {
      console.info('[landing] model-viewer loaded', {
        src: el.getAttribute('src'),
        time: new Date().toISOString(),
      })
    }
    console.info('[landing] model-viewer init', {
      src: el.getAttribute('src'),
      time: new Date().toISOString(),
    })
    el.addEventListener('load', handleLoaded as EventListener)
    return () => {
      el.removeEventListener('load', handleLoaded as EventListener)
    }
  }, [])
    ...
    >
      ...
      <div className="absolute inset-0 -z-10 overflow-hidden pointer-events-none">
        <ModelViewerElement
          ...
          poster=""
          style={{ width: '100%', height: '100%', background: 'transparent', opacity: 0.45 }}
        />
      </div>
      {/* Hairlines */}
